...
func convertToFullText(str []string) (res string) {
	for i := range str {
		...
		setQuery.Where(
			...
			MS.BEATMAPSET_ID IN ( 
				SELECT BEATMAPSET_ID FROM (
							  SELECT BEATMAPSET_ID FROM SEARCH_CACHE_ARTIST  WHERE @ARTIST  AND INDEX_KEY IN ( SELECT ID FROM SEARCH_CACHE_STRING_INDEX WHERE MATCH(STRING) AGAINST ( @TEXT IN BOOLEAN MODE ))
					UNION ALL SELECT BEATMAPSET_ID FROM SEARCH_CACHE_CREATOR WHERE @CREATOR AND INDEX_KEY IN ( SELECT ID FROM SEARCH_CACHE_STRING_INDEX WHERE MATCH(STRING) AGAINST ( @TEXT IN BOOLEAN MODE ))
					UNION ALL SELECT BEATMAPSET_ID FROM SEARCH_CACHE_TAG     WHERE @TAG     AND INDEX_KEY IN ( SELECT ID FROM SEARCH_CACHE_STRING_INDEX WHERE MATCH(STRING) AGAINST ( @TEXT IN BOOLEAN MODE ))
					UNION ALL SELECT BEATMAPSET_ID FROM SEARCH_CACHE_TITLE   WHERE @TITLE   AND INDEX_KEY IN ( SELECT ID FROM SEARCH_CACHE_STRING_INDEX WHERE MATCH(STRING) AGAINST ( @TEXT IN BOOLEAN MODE ))
					UNION ALL SELECT BEATMAPSET_ID FROM SEARCH_CACHE_OTHER   WHERE @OTHER   AND INDEX_KEY IN ( SELECT ID FROM SEARCH_CACHE_STRING_INDEX WHERE MATCH(STRING) AGAINST ( @TEXT IN BOOLEAN MODE ))
				) C group by BEATMAPSET_ID HAVING COUNT(1) >= @LEN
			)
`,
			sql.Named("TEXT", convertToFullText(text)),
